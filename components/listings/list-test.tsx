...
const ListingForm = ({ editMode = false, editItemId }: ListingFormProps) => {
  const [currentStep, setCurrentStep] = useState(0)
  const [isSubmitting, setIsSubmitting] = useState(false)
  console.log(editMode, editItemId)




  const { closeAddItem } = useUIState()
  const { itemListingFormData, setItemListingFormData, listedItems, setListedItems  } = useAppState()
  ...
  const [structuredSpecificDateSlots, setStructuredSpecificDateSlots] = useState<Array<{ date: string; timeSlots: string[] }>>([])

  // Add a flag to track if initialization has been done
  const [isInitialized, setIsInitialized] = useState(false)

  const steps = [
    { title: "Basic Details", schema: basicDetailsSchema },
    { title: "Categories", schema: categoriesSchema },
    { title: "Time Slot", schema: timeSlotSchema },
    { title: "Pickup Details", schema: pickupDetailsSchema },
  ]
  ...
  const getSafeFormData = useCallback(
    (): FormData => ({
      ...
      image: Array.isArray(itemListingFormData.image) ? itemListingFormData.image : [],
  ...
  const handleNext = async () => {
    ...
    if (isValid) {
      ...
      const updatedFormData = {
        ...
        image: convertedFormData.image?.length === 0 ? undefined : convertedFormData.image,
      }
  ...
  const handlePrevious = () => {
    if (currentStep > 0) {
      ...
      setItemListingFormData({
        ...updatedFormData,
        image: updatedFormData.image?.length === 0 ? undefined : updatedFormData.image,
      })
      setCurrentStep((prev) => prev - 1)
    }
  }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      // Create actual image URLs from uploaded files instead of placeholders
      const imageUrls = validatedData.image.map((file) => URL.createObjectURL(file))

      // Create a new listed item with "under-review" status
      const newItem = {
        id: Date.now().toString(),
        title: validatedData.title,
        description: validatedData.description,
        images: imageUrls, // Use actual uploaded images
  ...
  const handleSaveAsDraft = () => {
    ...
    // Create actual image URLs for the draft item if images exist
    const imageUrls =
      convertedData.image && convertedData.image.length > 0
        ? convertedData.image.map((file) => URL.createObjectURL(file))
        : ["/placeholder.svg?height=200&width=300&text=NoImage"]

    // Add the new item as draft
    const newItem = {
      id: Date.now().toString(),
      title: convertedData.title || "Untitled Item",
      description: convertedData.description || "No description",
      images: imageUrls, // Use actual uploaded images or single placeholder
      status: "draft" as const,
      expiresIn: "14 days",
      category: convertedData.category || "Other",
      condition: convertedData.condition || "Good",
      createdAt: new Date(),
    }
                    ...
                    >
                      ...
                      <div className="mt-4">
                        ...
                        <p className="mt-1 text-xs text-gray-500">
                          Upload clear photos of your item (up to 3) PNG, JPG, GIF up to 10MB each
                        </p>
  ...
  return (
    <>
      ...
      <div className="border-0 sm:border rounded-none sm:rounded-lg pb-4">
        ...
        <TimeSlotSelectModal open={showConfirmModal} onOpenChange={setShowConfirmModal}>
        ...
        {/* Success Modal */}
        <TimeSlotSelectModal onOpenChange={() => {}} open={showSuccessModal} >
          <div className="flex flex-col items-center justify-center text-center space-y-4 py-12">
            <Image
              src="/assets/icons/verification-in-progress.svg"
