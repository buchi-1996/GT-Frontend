...
const {setSidebarOpen} = useAppState()
  ...
  return (
    <header className="md:container sticky top-0 mx-auto px-6 lg:px-12 bg-[#ffffff] border-b border-gray-200 py-4 flex items-center justify-between">
      <div className='flex items-center gap-4'>
        <button onClick={() => setSidebarOpen(true)} className='cursor-pointer'>
          <Menu className='block sm:hidden' />
        </button>
        <h1 className="text-[1rem] sm:text-xl font-semibold text-[#222222]">Welcome back, Joe!</h1>
      </div>

      <div className="flex items-center gap-5">
        <Button className="hidden lg:flex bg-[#0d9488] hover:bg-[#0b5f5a]/90 cursor-pointer text-white px-6 py-5 rounded-md items-center gap-2 shadow-none">
